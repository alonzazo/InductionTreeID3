...
public class MLSystemManager {
	...
	public void run(String[] args) throws Exception {
		...
		if (evalMethod.equals("training"))
		{
			...
			ID3 id3 = new ID3();
			Matrix features = new Matrix(data, 0, 1, data.rows(), data.cols()-1);
			Matrix labels = new Matrix(data, 0, 0, data.rows(), 1);
			Scanner scan = new Scanner(System.in);
			TreeController tc = new TreeController(data);
			LinkedHashSet<String> trivia;
			int i = 0;
			while (i < 10){
				trivia = tc.getQuestionAndOptions();
				if (trivia.isEmpty()) break;
				System.out.println(trivia);
				String answer = scan.next();
				tc.reply(answer);
				i++;
			}
			tc.addAnswer("familia", "Insectiviris");
			//System.out.println(tc.getQuestionAndOptions());
			tc.getFeedbackMatrix().print();
			
			//Node root = id3.buildTree(features, labels);
			//imprimirRecursivo("",root, features, labels);
		}
...
//			System.out.println("Train Mean accuracy=" + (trainSumAccuracy / (reps * folds)));
	...
	public static void imprimirRecursivo(String espaciado,Node nodito, Matrix features, Matrix labels){
		//Si el nodo es una hoja
		if (nodito.getLabel().getValue() != -1){
			System.out.println(espaciado + nodito.getLabel().getStrValue());
		}
		else {//Si el nodo es interno
			System.out.println(espaciado + nodito.getAttribute().getName());
		}
		...
		if(!sub.isEmpty()){
			for (Double nodititito : sub.keySet()){
				...
				imprimirRecursivo(espaciado+"|   ",sub.get(nodititito), features, labels)	;
			}
		}
